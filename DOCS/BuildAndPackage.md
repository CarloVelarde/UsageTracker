...
6. Writes the packaged output to `dist/UsageTracker.exe`.
...
- main executable: `dist/UsageTracker.exe`
- latest build output: the single `dist/UsageTracker.exe` file
...
The package is a single-file build so the executable can be copied to the Desktop and run by double-clicking it. To archive versions, manually copy `dist/UsageTracker.exe` into a versioned folder such as `releases/UsageTrackerV1/UsageTrackerV1.exe`.
